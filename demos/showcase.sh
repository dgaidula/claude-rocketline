...
R5=$(( $(date +%s) + 12600 ))
J='{"model":{"display_name":"Opus 4.8 (1M context)"},"effort":{"level":"high"},"context_window":{"used_percentage":30},"rate_limits":{"five_hour":{"resets_at":'"$R5"'}},"cwd":"'"$DEMO"'","terminal":{"width":'"$W"'}}'
run() { printf '%s' "$J" | env "$@" bash "$SL"; printf '\n'; }

echo "── themes (flame caps) ──"
printf '  danny     '; run STATUSLINE_THEME=danny
printf '  dark-dan  '; run STATUSLINE_THEME=dark-dan
echo
echo "── cap styles (danny) ──"
for c in flame slant rounded pointed; do printf '  %-8s ' "$c"; run STATUSLINE_CAPS="$c"; done
